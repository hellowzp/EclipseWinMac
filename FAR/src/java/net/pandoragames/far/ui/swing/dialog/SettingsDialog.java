	...
	class SaveButtonListener implements ActionListener {
		public void actionPerformed(ActionEvent event) {
			if( canSave() ) {
				...
				boolean infoSetHasChanged = (! fileInfoOptions.getSelection().getActionCommand().equals( swingConfig.getDefaultFileInfo().name() )
										|| (fileInfoOptions.getSelection().getActionCommand().equals( SwingConfig.DefaultFileInfo.SIZE.name())
...
//				replaceForm.setCharacterset( defaultCharacterSet );
				...
				logger.info("Settings saved to: group_seperator=" + groupReference 
												+ "; do_backup=" + doBackup
												+ "; default_charset=" + defaultCharacterSet
												+ "; added_charsets=" + addedCharsets.size());
				SettingsDialog.this.dispose();
			}
		}
