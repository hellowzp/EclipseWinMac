...
public class Graph {
	...
	public void dijkstra(String startName) {
		...
		int nodesVisited = 0;
		while (!pq.isEmpty() && nodesVisited < vertexMap.size()) {
			Path vrec = pq.remove();
			Vertex v = vrec.dest;
			...
			for (Edge e : v.adj) {
				...
				if (cvw < 0)
					throw new PuzzleGraphException("Graph has negative edges");
	...
	public void negative(String startName) {
		...
		while (!q.isEmpty()) {
			...
			if (v.scratch++ > 2 * vertexMap.size())
				throw new PuzzleGraphException("Negative cycle detected");
	...
	public void acyclic(String startName) {
		...
		if (iterations != vertexMap.size())
			throw new PuzzleGraphException("Graph has a cycle!");
	}
